import React from 'react'
import './Footer.css'
import LaunchIcon  from '@mui/icons-material/Launch';
function Footer() {
    return (
        <div>
        <div className="fin">
        <div className="button">
            <button className="btn">Get Started</button>
        </div>

        <div className="dik">
            <div className="tal">Take the Tutorial</div>
        </div>
    </div>

    <div className="dy">
        <div className="darr">
            <div className="veer">
                <div className="image">
           <img alt="Facebook Open Source" src="data:image/png;base64," class="css-1izr7si"></img>
                <p className="foots">Copyright © 2021 Facebook Inc.</p>
                </div>
            </div>

         <div className="hit">
             <div className="nav">
                 <div className="foot">
                     <h3 className="head">DOCS</h3>
                
                 <div className="lin">
                     <li className="li">Installation</li>
                     <li className="li">Main Concepts</li>
                     <li className="li">Advanced Guides</li>
                     <li className="li">API Reference</li>
                     <li className="li">Hooks</li>
                     <li className="li">Testing</li>
                     <li className="li">Contributing</li>
                     <li className="li">FAQ</li>
                 </div>
                 </div>

                 <div className="foot">
                     <h3 className="head">CHANNELS</h3>
                
                 <div className="lin">
                     <li className="li">Github <LaunchIcon fontSize="small" className="searchIcon"/></li>
                     <li className="li">Stack Overflow </li>
                     <li className="li">Discussion Forums </li>
                     <li className="li">Reactiflux Chat </li>
                     <li className="li">DEV Community </li>
                     <li className="li">Facebook </li>
                     <li className="li">Twitter </li>
                   
                 </div>
                 </div>
             </div>

          
             <div className="nav">
                 <div className="foot">
                     <h3 className="head">COMMUNITY</h3>
                
                 <div className="lin">
                  <li className="li">Code of Conduct </li>
                     <li className="li">Community Resources</li>
             
                 </div>
                 </div>

                 <div className="foot">
                     <h3 className="head">MORE</h3>
                
                 <div className="lin">
                     <li className="li">Tutorial</li>
                     <li className="li">Blog</li>
                     <li className="li">Acknowledgements</li>
                     <li className="li">React Native</li>
                     <li className="li">Privacy</li>
                     <li className="li">Terms</li>
                   
                 </div>
                 </div>
             </div>
           
         </div>
        </div>
    </div>

    </div>  
       
    )
}

export default Footer
